...
# model = YOLO("runs/detect/train8/weights/best.pt")  # Load YOLOv8 model with pre-trained weights
...
while True:
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            cls = int(box.cls[0])
            if r.names[cls] == 'person' and confidence > 0.3:
                ...
                color = (255, 0, 0)
                thickness = 2
                cv2.putText(img, f'{ r.names[cls]} ,conf:{confidence}, xyxy:{x1},{y1},{x2},{y2}', org, font, fontScale, color, thickness)
